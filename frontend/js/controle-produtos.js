...
form.btnSalvar.addEventListener('click', (e) => {
    ...
    var quantidade = form.campoQuantidade.value;
    var valor = form.campoValor.value;
...
function criarTrComBaseNoProduto(produto){
    ...
    tdOpcoes.innerHTML = `<button onclick="editar(${produto.id})" class="btn btn-outline-info btn-sm">Editar</button>
                          <button onclick="deletar(${produto.id})" class="btn btn-outline-info btn-sm">Excluir</button>`
...
function adicionarProdutoNoBackEnd(produto){
    ...
    .then(resposta => resposta.json())
    .then(resposta => {
        var produto = new Produto(resposta);
...
function adicionarNovoProdutoATabela(produto){
    var tr = criarTrComBaseNoProduto(produto);
